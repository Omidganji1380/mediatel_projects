<div class="col">
    <div class="card">
        ...
        <p class="blog-detail">
            ...
            <span>
                <i class="far fa-bookmark"></i> {{ $post?->category?->locale_name }} </span>
        ...
        <div class="card-body bg-white  p-1 pt-2">
            <a href="{{ $post->link }}" class="card-title">
                <h4 class="blog-item">{!! strip_tags(Str::padRight($post->locale_title, 100)) !!}</h4>
            </a>
            <p class=" blog-meta">{!! $post->limit_content !!}</p>
            <div class="d-flex justify-content-between p-3">
                <small><i class="fa fa-eye"></i> {{ $post->views }}</small>
                <small><i class="fa fa-heart"></i> {{ $post->favorites_count }}</small>
            </div>
        </div>
    </div>
</div>
