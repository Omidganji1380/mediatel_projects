...
<div>
    ...
    <h1 style="text-align: {{ App::isLocale('fa') ? "right"  : "left"}};" dir="{{ App::isLocale('fa') ? "rtl"  : "ltr"}}">{!! __('emails.admin.rejected_ad', [
        'id' => $ad->id,
        'adminId' => $admin->id,
        'adminEmail' => $admin->email,
        'reason' => $reason ?: __('No Reason')
    ]) !!}</h1>
    <p style="text-align: {{ App::isLocale('fa') ? "right"  : "left"}};">{{ __('Title') }}: <b>{{ $ad->locale_title ?: $ad->description }}</b></p>
