...
trait VIP
{
    public function registerVIP(Api $t, Update $u): void
    {
        $this->sharePhoneForVIP($t, $u);
    }

    public function registerVIPGuide(Api $t, Update $u): void
    {
        // $text = "VIP User Guide Text";
        $text = st()->registerVIPUserGuideText;
        $this->sendMessageText($t, $u, $text);
    }

    public function sharePhoneForVIP(Api $t, Update $u)
    {
        ...
        $x->{$field} = $value;
        $user->update(['extra' => $x, 'telegram_last_method' => 'sharePhoneForVIP']);
    }

    public function storeVIP(Api $t, Update $u, Message|Collection|EditedMessage $m): void
    {
        $this->setLanguage();
        $this->registerUser($t, $u, $m, 'vip');
    }
}
