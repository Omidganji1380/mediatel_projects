...
trait RealEstate
{
    public function registerRealEstate(Api $t, Update $u): void
    {
        $this->sharePhoneForRealEstate($t, $u);
    }

    public function registerRealEstateGuide(Api $t, Update $u): void
    {
        // $text = "RealEstate User Guide Text";
        $text = st()->registerRealEstateUserGuideText;
        $this->sendMessageText($t, $u, $text);
    }

    public function sharePhoneForRealEstate(Api $t, Update $u)
    {
        ...
        $x->{$field} = $value;
        $user->update(['extra' => $x, 'telegram_last_method' => 'sharePhoneForRealEstate']);
    }

    public function storeRealEstate(Api $t, Update $u, Message|Collection|EditedMessage $m): void
    {
        $this->setLanguage();
        $this->registerUser($t, $u, $m, 'real_estate');
    }
}
