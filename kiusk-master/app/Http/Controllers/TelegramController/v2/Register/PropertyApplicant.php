...
trait PropertyApplicant
{
    public function registerPropertyApplicant(Api $t, Update $u): void
    {
        $this->sharePhoneForPropertyApplicant($t, $u);
    }

    public function registerPropertyApplicantGuide(Api $t, Update $u): void
    {
        // $text = "PropertyApplicant User Guide Text";
        $text = st()->registerPropertyApplicantUserGuideText;
        $this->sendMessageText($t, $u, $text);
    }

    public function sharePhoneForPropertyApplicant(Api $t, Update $u)
    {
        ...
        $x->{$field} = $value;
        $user->update(['extra' => $x, 'telegram_last_method' => 'sharePhoneForPropertyApplicant']);
    }

    public function storePropertyApplicant(Api $t, Update $u, Message|Collection|EditedMessage $m): void
    {
        $this->setLanguage();
        $this->registerUser($t, $u, $m, 'property_applicant');
    }
}
