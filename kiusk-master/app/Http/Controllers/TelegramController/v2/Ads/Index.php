...
trait Index
{
    ...
    public function inlineAdsList(Api $t, Update $u, Message|Collection|EditedMessage $m, $page = 1)
    {
        $this->updateUserExtra(function ($x) use ($page) {
            $x->listLastPage = $page;
    ...
    public function inlineAdsListBackToIt(Api $t, Update $u, Message|Collection|EditedMessage $m)
    {
        if (isset(auth()->user()->extra->listLastPage)) {
            $page = auth()->user()->extra->listLastPage;
        } else {
