...
class BlogController extends Controller
{
    public function index($page = 1)
    {
        ...
        $posts = Post::whereHas('category', function (Builder $q) {
    ...
    public function adCategory($slug, $page = 1)
    {
        ...
        $category = Category::whereSlug(urlencode($slug))
                            ->first();
        //  return
        $ads = Ad::whereHas('categories', function (Builder $q) use ($slug) {
    ...
    public function tags($slug, $page = 1)
    {
        ...
        $tag = Tag::where("slug->{$locale}", $slug)
                  ->whereType('post')
                  ->first();
        $ads = Post::whereHas('tags', function (Builder $q) use ($slug, $locale) {
